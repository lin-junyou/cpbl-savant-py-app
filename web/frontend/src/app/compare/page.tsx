...
export default function ComparePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">球員比較</h1>
        <p className="text-sm text-slate-700 mt-1">最多 4 位球員，PR 並列、雷達圖一目了然</p>
      </div>

      {/* Picker */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base text-slate-900">選擇球員（最多 4 位）</CardTitle>
        </CardHeader>
      ...
      {ids.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-slate-900">百分位 PR 對比</CardTitle>
          </CardHeader>
          <CardContent>
            <table className="w-full text-sm">
              <thead className="text-xs uppercase text-slate-700 border-b">
                <tr>
                  <th className="text-left py-2 pr-2">指標</th>
                  {profiles.map((q, i) => (
                    ...
                    {profiles.map((q, i) => {
                      ...
                      return (
                        <td key={i} className="py-1.5 px-2">
                          {v != null ? (
                            <PercentileBar label="" pr={v} value="" />
                          ) : (
      ...
      {ids.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base text-slate-900">原始數值對比</CardTitle>
          </CardHeader>
              ...
              <tbody>
                {[
                  ...
                  <tr key={key} className="border-b">
                    ...
                    {profiles.map((q, i) => {
                      const v = q.data?.season?.[key as string] as number | null;
                      let display = "—";
                      if (v != null) {
                        if (kind === "pct") display = (v * 100).toFixed(1) + "%";
                        else if (kind === "f3") display = v.toFixed(3);
                        else if (kind === "f1") display = v.toFixed(1);
                        else display = String(v);
                      }
                      return (
                        <td
                          key={i}
                          className="py-1.5 px-2 text-right tabular-nums text-slate-900"
                        >
                          {display}
                        </td>
                      );
